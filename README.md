...
- **股票详情**
  ...
  - 支持一键加入“虚拟交易”，加入后会自动生成该股票初始化交易策略，并执行一次按组合最优解的策略回放。
...
- **虚拟交易**
  - 第一次使用时输入虚拟满仓金额，应用会维护独立虚拟资金池、虚拟持仓和虚拟交易记录。
  - 可从股票详情页把当前股票加入虚拟交易股票池。
  - 新加入股票会自动拉取历史 K 线，生成一条初始化单股交易策略，并立即执行“按组合最优解回放”。
  - 策略优化默认使用最近 6 个月时间窗口，支持时间选择器调整回放区间。
  - 策略因子包含 K 线、MACD、SAR、BOLL、牛门线、量能和风控参数。
  - 虚拟股票池每只股票显示与策略优化监控一致的股票图表。
  - 虚拟股票池图表和从股票池打开的全屏图表只显示真实虚拟交易产生的买卖记录；策略优化监控图表显示历史回放产生的模拟交易点，两者互不混用。
  - A 股交易日开盘时段内，系统每 10 分钟按当前组合策略检查虚拟交易股票池，结合最新价格和 A 股交易规则执行模拟买卖。
  - 虚拟持仓和虚拟交易记录以表格展示，金额保留小数点后两位。
...
运行时会自动生成 `data/settings.json`、`data/cache.json`、`data/holdings.json`、`data/admin.json`、`data/tracking.json`、`data/virtual-trading.json`。这些文件可能包含 AK、缓存、持仓信息、管理员密码哈希、股票追踪池、虚拟交易资金池、虚拟持仓、虚拟交易记录、策略优化结果和历史回放数据，不建议提交到 GitLab。
...
- 管理员密码：用于保护历史持股数据，至少 6 位；ECS 部署时直接回车会使用默认密码 `123321`，部署完成后建议在设置页修改
- 模型供应商：通过 1-5 编号选择 `kimi-cn` / `kimi-intl` / `deepseek` / `minimax` / `glm`，脚本会回显已选择的供应商
...
- 自动虚拟交易只在 A 股交易日开盘时段生效。
- 每 10 分钟检查一次虚拟股票池。
- 只有真实虚拟交易产生的买卖记录会显示在虚拟股票池图表和对应全屏图表中。
- 历史策略回放产生的交易点只显示在“策略优化 / 模拟交易监控”里。
- 从股票详情页加入虚拟交易后，会立刻触发初始化策略和组合最优解回放，不需要等到下一个 10 分钟周期。
...
- 虚拟股票池图表只显示真实虚拟交易记录；如果当前虚拟交易记录为空，图表会显示 K 线但不会显示买卖点。
- 新加入股票会先生成初始化策略并执行组合最优解回放；这部分历史回放交易点会显示在“策略优化 / 模拟交易监控”，不会混到虚拟股票池图表中。
- 自动虚拟买卖只在 A 股交易日开盘时段每 10 分钟执行一次。
- 如果初始化策略失败，通常是该股票历史 K 线不足、行情源暂时不可用或网络请求失败，可稍后重新加入或运行策略优化。
